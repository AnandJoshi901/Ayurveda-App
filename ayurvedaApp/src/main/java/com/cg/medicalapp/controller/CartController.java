...
public class CartController {
    ...
    @GetMapping("/viewAllMedicine/{cartId}")
    ...
    public ResponseEntity<Cart> deleteMedicine(@PathVariable("cartId") int cartId)throws RecordNotFoundException
    {
    	return new ResponseEntity<Cart>(cartService.deleteMedicines(cartId), HttpStatus.OK);
    }
    
    @PostMapping("/placeOrder/{cartId}/{medicineId}")
    public ResponseEntity<Order> placeOrder(@PathVariable("cartId") int cartId,@PathVariable("medicineId") int medicineId){
